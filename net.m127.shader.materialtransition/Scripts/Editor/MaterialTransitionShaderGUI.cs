...
namespace M127
{
    public class MaterialTransitionShaderGUI : ShaderGUI
    {
        public struct StandardProps
        {
            public const string ALBEDO_NAME = "_MainTex",
                COLOR_NAME = "_Color",
                METALLIC_MAP_NAME = "_MetallicGlossMap",
                METALLIC_NAME = "_Metallic",
            ...
            public const string ALBEDO_TT = "Color of the surface",
                METALLIC_TT = "Metallicity Texture (R)",
                NORMAL_TT = "Normal Map",
                EMISSION_TT = "Emission map";

            public MaterialProperty albedo, color, metallicMap, metallic, smoothness, smoothness_channel, normal, normalScale, emission, emissionColor;
            private bool albedoFold, metallicFold, normalFold, emissionFold;
            ...
            public void swap(StandardProps other)
            {
                ...
                swapTexture(metallicMap, other.metallicMap);
                swapFloat(metallic, other.metallic);
            ...
            public void apply(MaterialEditor editor, out bool cnormal)
            {
                ...
                bool hasMetallic = metallicMap.textureValue != null;
                editor.TexturePropertySingleLine(EditorGUIUtility.TrTextContent(metallicMap.displayName, METALLIC_TT), metallicMap, metallic);
                editor.RangeProperty(smoothness, smoothness.displayName);
                if (hasMetallic)
                {
                    metallicFold = EditorGUILayout.Foldout(metallicFold, "Scale Offset");
                    if (metallicFold) editor.TextureScaleOffsetProperty(metallicMap);
                    editor.TextureCompatibilityWarning(metallic);
                }
